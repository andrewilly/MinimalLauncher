...
class HiddenAppsActivity : AppCompatActivity() {
    ...
    private lateinit var adapter: HiddenAppsListAdapter
        ...
        return LinearLayout(this).apply {
            ...
            addView(LinearLayout(context).apply {
                ...
                addView(TextView(context).apply {
                    text = getString(R.string.hidden_apps); textSize = 20f
    ...
    private fun loadApps() {
        lifecycleScope.launch {
            val apps = withContext(Dispatchers.IO) { AppUtils.loadLaunchableApps(this@HiddenAppsActivity) }
            allApps.clear(); allApps.addAll(apps)
            adapter = HiddenAppsListAdapter(allApps, prefs) { pkg, hidden ->
                Toast.makeText(this@HiddenAppsActivity,
                    if (hidden) "App nascosta" else "App mostrata", Toast.LENGTH_SHORT).show()
            }
...
class HiddenAppsListAdapter(
    private val apps: List<AppInfo>,
    private val prefs: PreferencesManager,
    private val onToggle: (String, Boolean) -> Unit
) : RecyclerView.Adapter<HiddenAppsListAdapter.ViewHolder>() {
